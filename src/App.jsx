...
export default function App() {
  ...
  const imgSrc = (path) => path ? `${BASE}${path}` : fallbackImage;
  ...
  const modalBoxStyle = {
    ...
    boxShadow: '0 0 30px rgba(0, 0, 0, 0.8)', color: '#f8fafc',
    position: 'relative', maxHeight: '90vh', overflowY: 'auto',
  };

  const closeBtnStyle = {
    position: 'absolute', top: '12px', right: '16px', fontSize: '1.6rem',
    ...
    return (
      <>
        ...
        <main className="intro-container">
          ...
          <p className="subtitle">An open-source OSINT repository detailing defence platforms, naval fleet specifications, and ground force assets.</p>
          <section className="purpose-card">
            <h2>Purpose of this Platform</h2>
            <p>This platform is engineered to serve as a consolidated educational and research dashboard. It brings together non-sensitive specifications sourced from open public domains (Wikipedia & OSINT) into a structured, modern interface.</p>
            <p><strong>Note:</strong> Access to the live interactive asset dashboard requires authentication.</p>
            <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem' }}>
              <button className="btn-primary" onClick={() => { setAuthMode('login'); setShowAuthModal(true); }}>Access Dashboard (Login)</button>
              <button className="btn-secondary" onClick={() => { setAuthMode('register'); setShowAuthModal(true); }}>Create Account</button>
            </div>
          </section>
        </main>
              ...
              {authMode === 'login' && (
                <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    <input type="text" required placeholder="Enter User ID" value={loginId} onChange={(e) => setLoginId(e.target.value)}
                  ...
                  <button type="submit" className="btn-primary" style={{ padding: '0.7rem', marginTop: '0.5rem' }}>Login</button>
                  <p style={{ textAlign: 'center', fontSize: '0.85rem', marginTop: '0.5rem' }}>
                    Don't have an account?{' '}
                    <a href="#" onClick={(e) => { e.preventDefault(); setAuthMode('register'); }} style={{ color: '#38bdf8', textDecoration: 'underline' }}>Register here</a>
                  </p>
                </form>
              )}
              {authMode === 'register' && (
                <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  ...
                  {regMsg.text && <div style={{ color: regMsg.type === 'error' ? '#ef4444' : '#10b981', fontSize: '0.9rem' }}>{regMsg.text}</div>}
                  <div>
                    <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem' }}>Full Name</label>
                    <input type="text" required placeholder="Enter Full Name" value={regName} onChange={(e) => setRegName(e.target.value)}
                      style={{ width: '100%', padding: '0.6rem', borderRadius: '6px', background: '#070d18', border: '1px solid #1e293b', color: '#fff' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem' }}>User ID</label>
                    <input type="text" required placeholder="Choose Unique User ID" value={regId} onChange={(e) => setRegId(e.target.value)}
                      style={{ width: '100%', padding: '0.6rem', borderRadius: '6px', background: '#070d18', border: '1px solid #1e293b', color: '#fff' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem' }}>Password</label>
                  ...
                  <button type="submit" className="btn-primary" style={{ padding: '0.7rem', marginTop: '0.5rem' }}>Register Account</button>
                  <p style={{ textAlign: 'center', fontSize: '0.85rem', marginTop: '0.5rem' }}>
                    Already registered?{' '}
                    <a href="#" onClick={(e) => { e.preventDefault(); setAuthMode('login'); }} style={{ color: '#38bdf8', textDecoration: 'underline' }}>Login here</a>
                  </p>
                </form>
              )}
            </div>
  ...
  return (
    <>
      ...
      <h1 style={{ marginTop: '2rem', textAlign: 'center', fontSize: '1.8rem', color: '#fff' }}>Secured Indian Defence Assets Dashboard</h1>
      <div className="subtitle">Sourced from open public domains (Wikipedia/OSINT). Displays non-sensitive specifications only.</div>

      <div className="dashboard-container" id="dashboard">
        {loadingAssets && <div style={{ textAlign: 'center', padding: '3rem', color: '#38bdf8' }}>Retrieving Secure Asset Database...</div>}
        {assetError && <div style={{ textAlign: 'center', color: '#ef4444', padding: '20px' }}><h2>Unable to load assets data</h2></div>}
      ...
      <div className="dashboard-container" style={{ marginTop: '2rem', marginBottom: '4rem' }}>
        ...
        <div style={{ background: '#0b1329', border: '1px solid #1e293b', borderRadius: '12px', padding: '1rem', overflowX: 'auto' }}>
          <table className="modal-table">
            <thead>
              <tr>
                <th>Request ID</th><th>Asset Name</th><th>Destination</th>
                <th>Quantity</th><th>Date</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {requisitions.length === 0 ? (
                <tr><td colSpan="6" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '2rem' }}>No active asset requisitions found.</td></tr>
              ) : (
        ...
        <div style={modalOverlayStyle} onClick={() => setSelectedAsset(null)}>
          <div style={{ ...modalBoxStyle, maxWidth: '520px' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="asset-image-container" style={{ borderRadius: '8px', marginBottom: '1rem', height: '220px' }}>
              <img
                src={imgSrc(selectedAsset.image)}
                alt={selectedAsset.name}
                style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                onError={(e) => { e.target.src = fallbackImage; }}
              />
            </div>
            <h2 style={{ textAlign: 'center', color: '#38bdf8', fontSize: '1.5rem', marginBottom: '0.2rem' }}>{selectedAsset.name}</h2>
            <p style={{ textAlign: 'center', color: '#94a3b8', fontSize: '0.9rem', marginBottom: '1.2rem' }}>{selectedAsset.type} — {selectedAsset.branch}</p>
            ...
            <div style={{ paddingTop: '1rem', borderTop: '1px solid #1e293b' }}>
              ...
              <form onSubmit={handleTransferRequest} style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.4rem', color: '#94a3b8', textAlign: 'center' }}>Destination Base Camp:</label>
                  <select required value={destBase} onChange={(e) => setDestBase(e.target.value)}
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.4rem', color: '#94a3b8', textAlign: 'center' }}>Units Required:</label>
                  <input type="number" min="1" max={selectedAsset.available_units || 16} value={transferQty}
                    onChange={(e) => setTransferQty(e.target.value)} required
                    style={{ width: '100%', padding: '0.65rem', borderRadius: '6px', background: '#070d18', color: '#fff', border: '1px solid #1e293b' }} />
                </div>
                <button type="submit" className="btn-primary" style={{ marginTop: '0.5rem', width: '100%', padding: '0.75rem' }}>Submit Requisition Request</button>
              </form>
            </div>
          </div>
        </div>
